...
/*
Returns every active membership tier with the current user's lowest eligible Stripe price.

API URL: GET /membership-tiers

Returns:

	tiers: active tiers with their selected price, pricing group, student status, and expiry date (HTTP 200)
...
/*
Expires any previous open Checkout Session and creates a new 30-minute Stripe Checkout Session.

API URL: POST /checkout/sessions
...
Raises:
	...
	401: user is not authenticated
	403: user has not completed onboarding
	409: user has an active membership or a payment is already processing
	422: tier has no eligible active Stripe price for the user
